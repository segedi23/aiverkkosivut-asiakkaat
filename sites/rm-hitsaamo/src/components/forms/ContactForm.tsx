  ...
  return (
    <div className="rounded-[2rem] border border-[var(--line)] bg-white p-6 shadow-[0_20px_60px_rgba(8,19,31,0.08)] sm:p-8">
      <div className="mb-5 grid gap-2">
        ...
        <h3 className="font-heading text-4xl leading-none text-[var(--text-strong)]">
          Lähetä viesti
        </h3>
        <p className="max-w-xl text-sm leading-7 text-[var(--text-soft)]">
          Kerro lyhyesti projektin tarve. Lomake avaa valmiin viestin oikeaan sähköpostiosoitteeseen.
        </p>
      </div>
      ...
      <form className="grid gap-4" onSubmit={handleSubmit}>
        ...
        <FieldWrapper label="Viesti">
          <TextArea
            required
            placeholder="Kerro kohteen tiedot, aikataulu ja mahdollinen kapasiteettitarve."
            value={message}
      ...
      {submitted ? (
        <p className="mt-4 rounded-2xl border border-[rgba(185,28,28,0.16)] bg-[var(--brand-blue-soft)] px-4 py-3 text-sm font-medium text-[var(--brand-blue)]">
          Sähköpostiohjelman pitäisi avautua valmiilla viestillä. Jos ei avaudu, ota yhteyttä puhelimitse.
        </p>
      ) : null}
    </div>
  )
